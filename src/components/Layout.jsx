...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    loadProgress()
  }, [location.pathname]) // Re-calculate on page navigation
  ...
  const getMotivationalMessage = (progress) => {
    ...
    if (progress < 50) return "You're making real progress!"
    if (progress < 75) return "Amazing work! Over halfway!"
    if (progress < 100) return "Almost there! You're incredible!"
    return "🎉 Journey Complete! You're a champion!"
  }
      ...
      <div className="p-4">
        <Card className="bg-gradient-to-r from-rose-50 to-pink-50 border-pink-200">
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold text-pink-600 mb-2">{journeyProgress}% Complete</div>
            <Progress value={journeyProgress} className="h-2" />
            <div className="text-xs text-gray-600 mt-2">{getMotivationalMessage(journeyProgress)}</div>
          </CardContent>
        </Card>
      </div>
    </div>
